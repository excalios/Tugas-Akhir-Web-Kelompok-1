                      ...
                      <div class="col">
                        ...
                        <select class="form-select" aria-label="Select category" name="category">
                          @foreach(['News', 'Feature', 'Guide', 'Release', 'Review'] as $category)
                            <option value="{{$category}}" @selected($category == $post->category->name)>{{$category}}</option>
                          @endforeach
                        </select>
                      </div>
                      <div class="col-1">
                        <label for="Rating" class="form-label">Rating</label>
                        <input type="number" class="form-control" name="rating" id="rating" aria-describedby="rating" value="{{ $post->rating }}">
                      </div>
                  </div>
                  <div class="mb-3">
